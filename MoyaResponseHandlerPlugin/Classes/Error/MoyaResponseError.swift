...
public enum MoyaResponseError: Swift.Error, LocalizedError {
    ...
    case networkingError(message: String, originalError: URLError, request: URLRequest?, response: URLResponse?)
    ...
    case moyaError(message: String, originalError: MoyaError, request: URLRequest?, response: URLResponse?)
    /// Not any of the URLError or Moya error types. Unknown error to the plugin.
    case otherError(message: String, originalError: Swift.Error, request: URLRequest?, response: URLResponse?)
    
    public var errorDescription: String? {
        switch self {
        case .networkingError(let message, _, _, _):
        ...
        case .moyaError(let message, _, _, _):
        ...
        case .otherError(let message, _, _, _):
